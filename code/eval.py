...
def read_ground_truth(gt_path):
    ground_truth_df = pd.read_csv(gt_path, sep='\t')  # Adjust delimiter based on ground truth file format
    if "contig" not in ground_truth_df.columns:
        raise ValueError(f"Ground truth file {gt_path} does not contain a 'contig' column.")
    return ground_truth_df[["contig"]].rename(columns={"contig": "Contig"})  # Standardize column name
...
def create_combined_dataframe(contigs_df, ground_truth_df, plasbinflow_df):
    ...
    # Merge contigs_df with PlasBinFlow and GT
    ...
    contigs_df["GT"] = contigs_df["Contig"].apply(
        lambda x: 1 if x in ground_truth_df["Contig"].values else 0
    )
...
input_file_path = "PlasBinFlow_Eval_Paths_with_seed.csv"
