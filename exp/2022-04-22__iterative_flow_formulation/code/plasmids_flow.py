...
if __name__ == "__main__":
	...
	while len(seeds_set) > 0:
		...
		contigs, ext = plasmids_preprocessing.contig_vars(m, contigs_dict, contigs, ext)
		...
		for e in links:	
			...
			if end1 != 'S' and end1 != 'T':
				m.addConstr(links[e] <= ext[end1], "link_ubd-"+str(e)+'-by-'+str(end1))
				constraint_count += 1
			if end2 != 'S' and end2 != 'T':	
				m.addConstr(links[e] <= ext[end2], "link_ubd-"+str(e)+'-by-'+str(end2))
				constraint_count += 1
		...
		for x in ext:
			b[x] = m.addVar(vtype=GRB.BINARY, name='aux-var-'+str(x))	
			expr = LinExpr()
			link_count = 0
			for link in extr_dict[x]:
				link_count += 1
				print("LOOP", x, link)
				expr.addTerms(1, links[link])
			m.addConstr(ext[x] <= expr, "extr_ubd-"+str(x)+'-by-expr')
			m.addConstr(ext[x] <= 1, "extr_ubd-"+str(x)+'-by-1')
			m.addConstr(ext[x] >= expr + M*b[x], "extr_lbd")
			m.addConstr(ext[x] >= 1 + M*(1-b[x]), "extr_lbd")
			constraint_count += 4

		#Constraint type 3
		#A contig 'c' is in the plasmid if at least one of its endpoints is in the plasmid.
		#Both endpoints should appear equal number of times in the plasmid.
		for c in contigs:
			end1, end2 = (c, 'h'), (c, 't')
			m.addConstr(contigs[c] == ext[end1], "contig-"+c+"-lbd-h")
			m.addConstr(contigs[c] == ext[end2], "contig-"+c+"-lbd-t")

			#m.addConstr(ext[end1] == ext[end2], "equal_extr_vals")
			constraint_count += 2
		...
		for c in counted_seed:
			m.addConstr(counted_seed[c] == contigs_dict[c]['Seed']*contigs[c], "counted_seed-"+c)
			print("SEED", c, str(contigs_dict[c]['Seed']))
			seed_expr.addTerms(1, counted_seed[c])
			constraint_count += 1
