...
The main requirements is that Segment entry (tag `S`) must include the
following fields
- `LN` (GFA1 standard field) encoding the segment length;
-`dp` (Unicycler specific field) representing the
  *normalized depth of coverage*, i.e. the depth of coverage of the
  segment normalized against the overall assembly graph depth of
  coverage.
